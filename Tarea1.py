...
def createColorSphere(N, R, r, g, b):
    ...
    L = np.zeros((N, N+1), dtype=int)  #matriz de ayuda
    for i in range(N):
        for j in range(N+1):
           L[i,j]=j+(N+1)*i
    for i in range(N):
        for j in range(N):
            if j==0:
                indices += [L[i,j+1], L[(i+1)%N,j+1], L[(i+1)%N,j]]
            elif j==N:
                indices += [L[i,j], L[i,j+1], L[(i+1)%N,j]]
            else:
                indices += [L[i,j], L[i,j+1], L[(i+1)%N,j], L[i,j+1], L[(i+1)%N,j+1], L[(i+1)%N,j]]
...
#anillos saturno:
gpuAnillo1 = createGPUShape(createColorRing(500,1.2,1.23,253/255,253/255,253/255)) 
gpuAnillo2 = createGPUShape(createColorRing(500,1.23,1.26,243/255,243/255,243/255)) 
gpuAnillo3 = createGPUShape(createColorRing(500,1.26,1.29,233/255,233/255,233/255)) 
gpuAnillo4 = createGPUShape(createColorRing(500,1.29,1.32,223/255,223/255,223/255)) 
gpuAnillo5 = createGPUShape(createColorRing(500,1.32,1.35,213/255,213/255,213/255)) 
